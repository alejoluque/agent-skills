...
export async function run(argv, context = {}) {
  ...
  try {
    ...
    if (registered.includes("claude")) {
      output.write("Claude: abre el proyecto y aprueba el MCP cuando lo solicite.\n");
    }
    if (registered.includes("codex")) {
      output.write("Codex: abre el proyecto, márcalo como confiable y comprueba la conexión con /mcp.\n");
    }
  } finally {
    prompter.close();
  }
